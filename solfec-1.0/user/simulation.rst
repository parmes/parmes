...
.. topic:: INITIALIZE_STATE (solfec, path, time | subset)
  ...
  This routine initializes the state of a Solfec-1.0 object with the state red from an output
  ...
  * solfec -- Solfec-1.0 object in the 'WRITE' mode
...
.. topic:: RIGID_TO_FEM (path, time, solfec | subset)
  ...
  This routine initializes the state of FEM bodies within the Solfec-1.0 object with the state of rigid bodies
...
.. topic:: CALLBACK (solfec, interval, data, callback)
  ...
  This routine defines a callback function, invoked during a run of Solfec-1.0 every interval of time.
...
.. topic:: WARNINGS (state)
  ...
  This routine disables or enables Solfec-1.0 warnings. It is a good practice to have the
...
.. topic:: IMBALANCE_TOLERANCE (solfec, tolerance | weightfactor, updatefreq)
  ...
  This routine sets the imbalance tolerance for parallel balancing of Solfec-1.0 data. A ratio of maximal to minimal
...
.. topic:: num = RANK ()
  ...
  This routine returns the rank of the CPU that runs the current copy of Solfec-1.0.
