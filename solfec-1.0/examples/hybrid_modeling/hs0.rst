.. _solfec-1.0-examples-hybrid_modeling-hs0:

A two--body impact problem
==========================

This is a simplest application of the :ref:`HYBRID_SOLVER <solfec-1.0-command-HYBRID_SOLVER>`. The input files for this example are
...
- `hs0--solfec--1.py <https://github.com/tkoziara/solfec/blob/master/examples/hybrid-solver0/hs0-solfec-1.py>`_ -- including the simpler version of the :ref:`Solfec-1.0 <solfec-1.0-index>` input code
...
.. note:: For the sake of using hs0--parmec.py as Parmec input in Solfec-1.0's :ref:`HYBRID_SOLVER <solfec-1.0-command-HYBRID_SOLVER>`,
...
:ref:`SOLFEC object <solfec-1.0-user-solfec>` is created in line 3, specifying the output directory as 'out/hybrid--solver0' (relative where *solfec* is run from).
:ref:`Gravity <solfec-1.0-command-GRAVITY>` is applied in line 5. Volumetric :ref:`"bulk" material <solfec-1.0-command-BULK_MATERIAL>` is created in line 7
and :ref:`"suraface material" <solfec-1.0-command-SURFACE_MATERIAL>` for contact interactions is created in line 10. Then the lower :ref:`body <solfec-1.0-user-body>` "bod1"
...
:ref:`MESH <solfec-1.0-command-MESH>` object is used to define geometry. 
...
The :ref:`Newton solver <solfec-1.0-command-NEWTON_SOLVER>` is created next in line 28. It will be used internally by the :ref:`hybrid solver <solfec-1.0-command-HYBRID_SOLVER>`
in order to resolve (non--smooth) contact interactions between the two bodies. The :ref:`HYBRID_SOLVER <solfec-1.0-command-HYBRID_SOLVER>` itself is created further, in line 32.
...
simulation. In lines 35 and 36 we define the Solfec-1.0 output interval. Because the :ref:`"OUTPUT" <solfec-1.0-command-OUTPUT>` commands :ref:`coincide <parmec-command-OUTPUT>`
in both codes, we need to explicitly call the Solfec-1.0 command in this case. The input file is concluded with the :ref:`"RUN" command <solfec-1.0-command-RUN>` (line 38),
...
This input file demonstrates several more elaborate features of the :ref:`HYBRID_SOLVER <solfec-1.0-command-HYBRID_SOLVER>` and of Solfec-1.0 input usage in general:
...
<solfec-1.0-command-CALLBACK>`, as demonstrated in lines 42--51. These time histories are later used, in lines 59--73, in order to
...
:ref:`XDMF export <solfec-1.0-xdmf>` is demonstrated in lines 75--80. We note, that Solfec-1.0 needs to be run for the second time,
