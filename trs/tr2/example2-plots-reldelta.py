...
process = subprocess.Popen('unzip -o example2-data.zip', shell=True)
process.wait()
...
for rldl in reldelta:
  ...
  for kin in kinem:
    for case in cases:
      ...
      V_X_Y = {}
      ic = cases.index(case)
      with open('example2-data/tr2-cubes25-reldelta-iters', 'r') as inp:
	ln = inp.readline()
	ln = ln.split(' = ')
	while ln != ['']:
	  name = ln[0].split('_')
	...
	for pnt in V_X_Y:
	  ...
	  Z[j][i] = float(d[k])
	plt.figure()
	if np.amax(Z) > 0: plt.scatter(X, Y, c=Z, edgecolors='none', s=200, norm=colors.LogNorm(), cmap=cm.jet)
	...
	plt.savefig ('cubes25_%s_%s%d_%s.png' % (kin, case, k, rldl), bbox_inches='tight', pad_inches=0)
	plt.close()
  ...
  for kin in kinem:
    for case in cases:
      ...
      V_X_Y = {}
      ic = cases.index(case)
      with open('example2-data/tr2-cubes25-reldelta-runtimes', 'r') as inp:
	ln = inp.readline()
	ln = ln.split(' = ')
	while ln != ['']:
	  name = ln[0].split('_')
	...
	Z[j][i] = V_X_Y[pnt]
      ...
      plt.xticks (xlist, [str(x) for x in V_X])
      plt.yticks (ylist, [str(y) for y in V_Y])
      plt.savefig ('cubes25_%s_%s_%s_runtimes.png' % (kin, case, rldl), bbox_inches='tight', pad_inches=0)
      plt.close()
  ...
  for kin in kinem:
    for case in cases:
      ...
      V_X_Y = {}
      ic = cases.index(case)
      with open('data/tr2-cubes25-reldelta-timings', 'r') as inp:
	ln = inp.readline()
	ln = ln.split(' = ')
	while ln != ['']:
	  name = ln[0].split('_')
	...
	Z[j][i] = V_X_Y[pnt]
      ...
      plt.xticks (xlist, [str(x) for x in V_X])
      plt.yticks (ylist, [str(y) for y in V_Y])
      plt.savefig ('cubes25_%s_%s_%s_ratios.png' % (kin, case, rldl), bbox_inches='tight', pad_inches=0)
      plt.close()
  '''

print 'Cleaning up...'
process = subprocess.Popen('rm -fr example2-data', shell=True)
process.wait()
