...
The Solfec-only approach takes about 18 minutes to complete calculations. The most simplified Parmec-only approach takes only
...
and finite element bodies (modeled in Solfec) takes about 5 minutes to complete. This is 3.4 times faster than the Solfec-only
case, and 31 times slower than Parmec-only. All Solfec calculations were done using 4 MPI processes, while Parmec calculations
