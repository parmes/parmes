.. _solfec-examples-hybrid_modeling-hs123:

1,2,3--dimensional cube array acceleration dwell
================================================

This is a family of 1,2 and 3--dismenional examples demonstrating applications of the :ref:`HYBRID_SOLVER <solfec-command-HYBRID_SOLVER>`
...
- `hs2--solfec--only.py <https://github.com/tkoziara/solfec/blob/master/examples/hybrid-solver2/hs2-solfec-only.py>`_ -- including the Solfec-only version of the example
...
:ref:`bulk <solfec-command-BULK_MATERIAL>` and :ref:`surface <solfec-command-SURFACE_MATERIAL>` materials, and a unit
...
bodies are created, based on a 2x2x2 :ref:`hexahedral mesh <solfec-command-HEX>`. The z--direction of the bodies is :ref:`fixed <solfec-command-FIX_DIRECTION>`
...
reside on other processor ranks. Hence we test this using the :ref:`HERE command <solfec-command-HERE>` and accordingly create
...
of the :ref:`HYBRID_SOLVER <solfec-command-HYBRID_SOLVER>`. In line 56 the Parmec body index is advanced, as this line corresponds
...
description of :numref:`hs0-lst3`, in the context of the :ref:`two body impact example <solfec-examples-hybrid_modeling-hs0>`.
...
did not attempt to consistently model a uniform physical system by means of the three approaches: Solfec-only, Parmec-only and hybrid.
The physical parameters of the finite element (FE) Solfec-only model do not match those of the simplified Parmec-only model: the FE
...
Solfec-only model has quite different dynamic response when compared with the latter two approaches. The Parmec-only and hybrid approaches
...
The Solfec-only approach takes about 38 minutes to complete calculations. The most simplified Parmec-only approach takes only
10 seconds and is 230 times faster. The hybrid approach, on the other hard, which includes both rigid bodies (modeled in Parmec)
and finite element bodies (modeled in Solfec) takes 6.7 seconds to complete. This is 5.7 times faster than the Solfec-only case,
and 40 times slower than Parmec-only. All Solfec calculations were done in sequential mode, while Parmec calculations took full
advantage of a 4-core 2.3 GHz Intel Core i7 CPU on a MacBook Pro laptop (mid 2012 model).

.. _hs2-runtimes:

.. table:: 2D array runtime comparison, between the NSCD (Solfec), DEM (Parmec) and hybrid approaches.

  +---------------+-------------+--------------+--------------+
  | Approach      | Solfec-only | Parmec-only  |  Hybrid      |
  +---------------+-------------+--------------+--------------+
  | Runtime [sec] | 2300        | 10           | 402          |
  +---------------+-------------+--------------+--------------+
