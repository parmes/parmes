...
1. A linear programming based limit--state formulation is employed in [1]_, whereas the dynamic contact algorithm is used in Solfec.
2. The analysis provided in [1]_ is two--dimensional, whereas Solfec deals with a three--dimensional model.
3. A node to face contact model is employed in [1]_, whereas the face to face (or more generally element to element) contact model is employed in Solfec.
...
adopted in the Solfec model. In order to geometrically capture the hinging effect from the first moments of simulation, the subdivision along the block
...
.. figure:: masonry_arch/arch-model.png
   ...
   The three--dimensional arch model in Solfec. Each of the 27 blocks is composed of 6 elements: two along the width :math:`w`,
...
lines coincide. :numref:`masonry_arch3` also includes the failure modes I, II, and III reproduced from three longer Solfec simulations
...
.. figure:: masonry_arch/arch-energy-compare.png
   ...
   Level curves from :numref:`masonry_arch2` obtained from Solfec superimposed with :numref:`masonry_arch0` from [1].
